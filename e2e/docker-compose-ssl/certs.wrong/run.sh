...
SERVER_ADDRESS_IP="10.116.8.52 192.168.8.202"
SERVER_ADDRESS_DNS="haproxy.nebula.svc.cluster.local"
CLIENT_ADDRESS_IP=""
CLIENT_ADDRESS_DNS=""
...
EOF
	...
	if [ ${cert_type} == "ca" ]; then
		...
		openssl x509 -req -in ${cert_type}.csr -out ${cert_type}.crt -CA ca.crt -CAkey ca.key -CAcreateserial -days 3650 -extfile ${cert_type}.cnf -extensions req_ext
	fi

}

cert_type=${1}
if [ ${cert_type} != "ca" ] && [ ! -e ca.crt ];then
	echo "ca.crt not exist"
	exit 1
fi
