...
func init() {
	common.File_nebula_common_proto.Options().ProtoReflect().Range(func(fd protoreflect.FieldDescriptor, v protoreflect.Value) bool {
		switch fd.FullName() {
		case protoreflect.FullName(common.E_ProtocolVersion.Name):
			f := common.File_nebula_common_proto.Options().ProtoReflect().Get(fd)
			i := f.Interface()
			if _, ok := i.([]byte); ok {
